...
Inputs (read-only), in either layout:

  native  <run>/raw/metrics, <run>/runtime/data, <run>/manifest.json and the
          level JSON + .gml the run adapter wrote under <run>/analysis-compat/
  legacy  esperimenti/<run>/<area>/{run/metrics, run/data, shadow.yaml} and
          config/levels/<area>.json

Outputs: config.csv, manifest.json and the tables listed in TABLES.

    python3 -m experiments.analysis.pipeline.phase1_collect --out <results>/_pipeline
"""
...
def collect_topology(run, cfg_root, yaml_info):
    ...
    # A native run carries its own level JSON, written by the run adapter, so
    # the campaign-wide config root is only consulted for legacy archives.
    own = Path(run["path"]) / "analysis-compat" / "levels" / ("%s.json" % run["livello"])
    level_json = own if own.is_file() else Path(cfg_root) / "levels" / ("%s.json" % run["livello"])
    gml = yaml_info.get("gml_path") or ""
    if gml and not Path(gml).is_file():
        # A legacy shadow.yaml stores the absolute path of the machine that ran
        # the campaign: fall back to the sibling <area>/ directory of that tree.
...
def build_config_rows(run, meta, params_host, startup, yaml_info, final_height, rtt_max_miners, rtt_max_all, gml, level_json):
    ...
    # These four keep their historical NAMES: they are row values in
    # config.csv and every archived analysis and report keys on them. Only the
    # stated source differs, and only for a native run - re-analysing the
    # archived campaign must still reproduce its config.csv byte for byte.
    native = yaml_info.get("layout") == "native"
    add("shadow_general_seed", yaml_info.get("general_seed", ""),
        "manifest.json seed" if native else "shadow.yaml general.seed")
    env = yaml_info.get("env", {})
    add("poesia_rng_seed", env.get("POESIA_RNG_SEED", ""),
        "harness environment POESIA_RNG_SEED" if native else "shadow.yaml env POESIA_RNG_SEED")
    add("poesia_epochlen", env.get("POESIA_EPOCHLEN", ""),
        "harness environment POESIA_EPOCHLEN" if native else "shadow.yaml env POESIA_EPOCHLEN")
    add("topology_gml", gml,
        "the run's exported topology .gml" if native else "shadow.yaml network.graph.file.path")
    if native:
        # New rows, appended: additive changes only, so a reader keyed on the
        # historical names is unaffected.
        add("harness_backend", (yaml_info.get("env", {}) or {}).get("POESIA_BACKEND", ""),
            "manifest.json fabric_backend")
...
def collect_run(run, out_root, cfg_root):
    ...
    yaml_info = C.parse_run_metadata(run)
    ...
    manifest = OrderedDict([
        ...
        # Historical section name. It holds the harness metadata whatever the
        # backend was; phase 2, phase 3 and every archived report read it by
        # this key, so renaming it would break them for no gain.
...
def main(argv=None):
    ...
    ap.add_argument("--root", default="", help="runs to analyse (read-only)")
    ...
    root = Path(args.root).expanduser().resolve() if args.root else results_root()
    out_root = Path(args.out).expanduser().resolve() if args.out else results_root() / "_pipeline"
    cfg_root = Path(args.config_root).expanduser().resolve() if args.config_root else CONFIG_ROOT
    ...
    for run in runs:
        try:
            collect_run(run, out_root, cfg_root)
