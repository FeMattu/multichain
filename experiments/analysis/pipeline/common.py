...
def discover_runs(root):
    """All runs under `root`, in either layout.

    Two trees are understood and the choice is made by inspection, never by a
    flag, so that a campaign mixing archived and freshly produced runs still
    analyses in one pass:

    * NATIVE (this harness): `<root>/<run-id>/` carrying `manifest.json` and
      `raw/metrics/`.
    * LEGACY (the Shadow archive): `<root>/runN-tbt-Ts/<level>/run/metrics/`.

    The record shape is the same either way, `livello` included - it is a
    column in every historical CSV, and renaming it would break the very
    comparison this migration exists to preserve.
    """
    native = _discover_native(root)
    if native:
        return native
    found = []
    if not root.is_dir():
...
def _discover_native(root):
    """Runs produced by the CORE/netns harness. Empty when there are none."""
    try:
        from ..compatibility.run_adapter import discover_native_runs
    except ImportError:                                   # pragma: no cover
        return []
    if not Path(root).is_dir():
        return []
    return discover_native_runs(Path(root))


def parse_run_metadata(run):
    """Seed, GML path and harness environment, from whichever layout applies.

    A native run carries them in `manifest.json` plus the compatibility inputs
    the adapter wrote; a legacy one in `shadow.yaml`. Callers should not have
    to know which.
    """
    path = run["path"] if isinstance(run, dict) else Path(run)
    compat = Path(path) / "analysis-compat" / "run-metadata.json"
    if compat.is_file():
        data = read_json(compat) or {}
        return {"general_seed": data.get("general_seed", ""),
                "gml_path": data.get("gml_path", ""),
                "env": data.get("env", {}),
                "layout": "native"}
    manifest = Path(path) / "manifest.json"
    if manifest.is_file():
        data = read_json(manifest) or {}
        return {"general_seed": data.get("seed", ""),
                "gml_path": str(Path(path) / "config" / "topology.gml"),
                "layout": "native",
                "env": {"POESIA_RNG_SEED": str(data.get("seed", "")),
                        "POESIA_EPOCHLEN": str(data.get("epoch_length_blocks", "")),
                        "POESIA_SETUPBLOCKS": str(data.get("setup_first_blocks", "")),
                        "POESIA_CHAIN": data.get("chain", "")}}
    legacy = parse_shadow_yaml(Path(path) / "shadow.yaml")
