...
| `fork_score_log` | **`true`** | `-debug=wpoafork`: per contested round, every candidate with its true score and arrival order, the reconstructed first-seen winner, and the score-aware activation's holds (`[wpoa-fork] defer`, `defer-release`). Cheap: it logs per fork-choice *candidate*, not per registry row. The fork choice itself has **no key**: under private sortition the node always breaks same-height ties on the true score, because the score-aware activation depends on it. The former `fork_score` key is refused, so a profile cannot believe it runs a control arm that no longer exists. |
