...
static const mc_OneMultichainParam MultichainParamArray[] =
{
    ...
    { "enablewpoa"                        , "enable-wpoa"                 ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoa",
                   "enablewpoaweights","wPoA weighted proof-of-authority (permissioned miner selection)",
                   "Master switch: enable the whole wPoA protocol (weights stream + weighted selection + VRF + RANDAO + sortition). More specific enable-wpoa-* flags override it per phase. Default 0 (native MultiChain mining)."},
    { "enablewpoaweights"                 , "enable-wpoa-weights"         ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoaweights",
                   "enablewpoaselection","",
                   "wPoA Phase 1: run the append-only wpoa-weights stream (validators register their weight). Forced on by enable-wpoa and required by weighted selection. Default 0."},
    { "enablewpoaselection"               , "enable-wpoa-selection"       ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoaselection",
                   "dumpfunction","",
                   "wPoA Phase 2: weighted proposer selection (Efraimidis-Spirakis draw over the weights stream). Requires enable-wpoa-weights. Default 0."},
    { "dumpfunction"                      , "dump-function"               ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 16,         0,          0,         0, 0.0, 20014, 0, "-mc-dumpfunction",
                   "enablewpoavrf","",
                   "wPoA weight-dumping (damping) function applied before selection: none, sqrt or log. Consensus-critical: must be identical on all nodes. Default none."},
    { "enablewpoavrf"                     , "enable-wpoa-vrf"             ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoavrf",
                   "enablewpoarandao","",
                   "wPoA Phase 3a: VRF randomness beacon; each elected proposer publishes a verifiable reveal, verified by peers. Requires enable-wpoa-selection. Default 0."},
    { "enablewpoarandao"                  , "enable-wpoa-randao"          ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoarandao",
                   "wpoarandaolookback","",
                   "wPoA Phase 3b: RANDAO beacon seed seed[n+1]=H(R_tot[n-k]|h[n]|n+1) from accumulated VRF reveals. Requires enable-wpoa-vrf. Default 0."},
    { "wpoarandaolookback"                , "wpoa-randao-lookback"        ,
                   MC_PRM_UINT32  | MC_PRM_USER | MC_PRM_CLONE  , -1,         1,          0,   1000000, 0.0, 20014, 0, "-mc-wpoarandaolookback",
                   "enablewpoasortition","",
                   "wPoA RANDAO lookback distance k. Must be identical on all nodes; sortition requires k>=1. Default 1."},
    { "enablewpoasortition"               , "enable-wpoa-sortition"       ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoasortition",
                   "wpoasortitiondelta","",
                   "wPoA Phase 4: private (VRF-scored) sortition; each validator self-elects via a score-proportional mining delay, so the proposer is unpredictable until it acts. Requires enable-wpoa-randao and lookback k>=1. Default 0."},
    { "wpoasortitiondelta"                , "wpoa-sortition-delta"        ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoasortitiondelta",
                   "wpoasortitionlambda","",
                   "wPoA sortition delay band half-width as a fraction of target-block-time, delta in (0,1). Small: narrower band, more forks; large: more latency. Consensus-critical: identical on all nodes. Default 0.5."},
    { "wpoasortitionlambda"               , "wpoa-sortition-lambda"       ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoasortitionlambda",
                   "enablewpoamalus","",
                   "wPoA sortition feedback gain lambda in [0,1]: weight of the global correction Phi recentring the mean block time on target. 0 disables it. Consensus-critical: identical on all nodes. Default 0."},
    { "enablewpoamalus"                   , "enable-wpoa-malus"           ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enablewpoamalus",
                   "wpoamalusmu","",
                   "wPoA behavioural malus: run the open wpoa-weights-malus report stream and elect on the effective weight w_eff = w * Psi. Requires enable-wpoa-sortition. Consensus-critical: identical on all nodes. Default 0."},
    { "wpoamalusmu"                       , "wpoa-malus-mu"               ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoamalusmu",
                   "wpoamalusmax","",
                   "wPoA malus accumulator persistence mu in [0,1): the fraction of M carried into the next epoch. mu < 1 is what makes an exclusion reversible. Consensus-critical: identical on all nodes. Default 0.5."},
    { "wpoamalusmax"                      , "wpoa-malus-max"              ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoamalusmax",
                   "wpoamalusequivpoints","",
                   "wPoA malus threshold M_max > 0: the accumulator value at which Psi reaches 0 and the validator becomes ineligible. Consensus-critical: identical on all nodes. Default 4."},
    { "wpoamalusequivpoints"              , "wpoa-malus-equiv-points"     ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoamalusequivpoints",
                   "wpoamalusdelaypoints","",
                   "wPoA malus score added by one proved equivocation, a safety fault. Must exceed the delay score. Consensus-critical: identical on all nodes. Default 4."},
    { "wpoamalusdelaypoints"              , "wpoa-malus-delay-points"     ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoamalusdelaypoints",
                   "wpoamalusselfwritepoints","",
                   "wPoA malus score added by one proved scheduling-delay violation. Consensus-critical: identical on all nodes. Default 0.25."},
    { "wpoamalusselfwritepoints"          , "wpoa-malus-selfwrite-points" ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoamalusselfwritepoints",
                   "wpoamalusbadweightpoints","",
                   "wPoA malus score for publishing a self-attested record on another address's behalf. The record is always discarded, so this prices the attempt. Consensus-critical: identical on all nodes. Default 1."},
    { "wpoamalusbadweightpoints"          , "wpoa-malus-badweight-points" ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-wpoamalusbadweightpoints",
                   "enableweightengine","",
                   "wPoA malus score for a wpoa-weights value failing independent recomputation. Must exceed the selfwrite score: a false weight succeeds unless recomputed. Consensus-critical. Default 2."},

    { "enableweightengine"                , "enable-weight-engine"        ,
                   MC_PRM_BOOLEAN | MC_PRM_USER | MC_PRM_CLONE  , -1,         0,          0,         0, 0.0, 20014, 0, "-mc-enableweightengine",
                   "weightepochlength","weight engine (dynamic ESG-derived validator weights)",
                   "Weight engine: derive each cluster's wpoa-weights value from on-chain inputs (membership/ESG/activity/reconciliation) per epoch, instead of a static -weight. Requires enable-wpoa-weights. Consensus-critical: identical on all nodes. Default 0."},
    { "weightepochlength"                 , "weight-epoch-length"         ,
                   MC_PRM_UINT32  | MC_PRM_USER | MC_PRM_CLONE  , -1,       100,          1,   1000000, 0.0, 20014, 0, "-mc-weightepochlength",
                   "weightkappa","",
                   "Weight engine epoch length in blocks: epoch(height) = height / this. Consensus-critical: must be identical on all nodes. Default 100."},
    { "weightkappa"                       , "weight-kappa"                ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-weightkappa",
                   "weightalpha","",
                   "Weight engine normalization constant kappa > 0 in the company contribution c_i = ESG_i * tau_i / kappa. Consensus-critical: must be identical on all nodes. Default 100."},
    { "weightalpha"                       , "weight-alpha"                ,
                   MC_PRM_STRING  | MC_PRM_USER | MC_PRM_CLONE  , 32,         0,          0,         0, 0.0, 20014, 0, "-mc-weightalpha",
                   "weightlambda","",
