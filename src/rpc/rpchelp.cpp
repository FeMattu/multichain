...
void mc_InitRPCHelpMap26()
{
    // --- wPoA round audit: score, delay, and the two weight stages ---
    mapHelpStrings.insert(std::make_pair("wpoagetlocalscore",
            "wpoagetlocalscore ( height )\n"
            "\n"
            "Returns this node's wPoA election score for the round at `height`.\n"
            "\n"
            "AUDIT, NOT PREDICTION. This is the PUBLIC Efraimidis-Spirakis form, whose entropy\n"
            "is HMAC-SHA256(seed, address) — the only form recomputable without a validator's\n"
            "secret key. Where private sortition governs the height the real election runs the\n"
            "same transform over a per-validator VRF output instead, so these scores audit the\n"
            "model and its inputs; they do not reveal the actual proposer, which stays unknown\n"
            "until that node proposes.\n"
            "\n"
            "Arguments:\n"
            "1. height                          (numeric, optional, default=tip+1) the round n+1 to\n"
            "                                    audit. Must be <= tip+1: the beacon seed of a\n"
            "                                    later round does not exist yet.\n"
            "\n"
            "Result:\n"
            "{\n"
            "  \"score\" : x,                       (numeric) score_i = -ln(u_i)/g(w_eff_i); null when\n"
            "                                               the validator is ineligible (score = +inf)\n"
            "  \"weight\" : n,                      (numeric) w_i, the raw registry weight\n"
            "  \"effective_weight\" : n,            (numeric) w_eff_i = w_i * Psi_i (behavioural malus)\n"
            "  \"eligible\" : true|false,           (boolean) whether w_eff_i > 0, i.e. i is in V+,\n"
            "  \"address\" : \"address\",             (string) this node's validator address\n"
            "  \"height\" : n,                      (numeric) the round n+1 these values are for\n"
            "  \"seed\" : \"hex\",                    (string) the 32-byte seed the scores are drawn over\n"
            "  \"seed_source\" : \"randao|prevblockhash\", (string) which beacon produced it\n"
            "  \"dumping_function\" : \"none|sqrt|log\",   (string) g, the active dumping function\n"
            "  \"total_effective_weight\" : x       (numeric) W = sum_j g(w_eff_j), the band normalizer\n"
            "}\n"
            + HelpExampleCli("wpoagetlocalscore", "")
            + HelpExampleRpc("wpoagetlocalscore", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetnodescore",
            "wpoagetnodescore \"address\" ( height )\n"
            "\n"
            "Returns the wPoA election score of one validator for the round at `height`.\n"
            "\n"
            "AUDIT, NOT PREDICTION. This is the PUBLIC Efraimidis-Spirakis form, whose entropy\n"
            "is HMAC-SHA256(seed, address) — the only form recomputable without a validator's\n"
            "secret key. Where private sortition governs the height the real election runs the\n"
            ...
            "2. height                          (numeric, optional, default=tip+1) the round n+1 to\n"
            "                                    audit. Must be <= tip+1: the beacon seed of a\n"
            "                                    later round does not exist yet.\n"
            "\n"
            "Result:\n"
            ...
            "  \"total_effective_weight\" : x       (numeric) W = sum_j g(w_eff_j), the band normalizer\n"
            "}\n"
            + HelpExampleCli("wpoagetnodescore", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("wpoagetnodescore", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("wpoalistscores",
            "wpoalistscores ( height )\n"
            "\n"
            "Returns the wPoA election score of EVERY validator in the weight registry for the\n"
            "round at `height`, in ascending address order — the same iteration the proposer\n"
            "argmin uses, so the comparison can be replayed in the printed order.\n"
            "\n"
            "AUDIT, NOT PREDICTION. This is the PUBLIC Efraimidis-Spirakis form, whose entropy\n"
            "is HMAC-SHA256(seed, address) — the only form recomputable without a validator's\n"
            "secret key. Where private sortition governs the height the real election runs the\n"
            ...
            "Result:\n"
            "{\n"
            "  \"validators\" : n,                  (numeric) how many validators were scored\n"
            "  \"scores\" : {                       (object) address -> score entry\n"
            "    \"address\" : {\n"
            "      \"score\" : x, \"weight\" : n, \"effective_weight\" : n, \"eligible\" : true|false\n"
            "    }, ...\n"
            "  },\n"
            "  \"height\" : n,                      (numeric) the round n+1 these values are for\n"
            "  \"seed\" : \"hex\",                    (string) the 32-byte seed the scores are drawn over\n"
            ...
            "  \"total_effective_weight\" : x       (numeric) W = sum_j g(w_eff_j), the band normalizer\n"
            "}\n"
            + HelpExampleCli("wpoalistscores", "")
            + HelpExampleRpc("wpoalistscores", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetlocaldelay",
            "wpoagetlocaldelay ( height )\n"
            "\n"
            "Returns this node's mining delay for the round at `height`: the timer its score\n"
            "buys it, and the terms the timer is built from.\n"
            "\n"
            "D_i = T_block + delta*T_block*(2*score_norm_i - 1) + lambda*Phi^(n). The middle\n"
            "term orders the candidates and only that; Phi is identical for every candidate of\n"
            "the round, so it shifts all timers together and can never reorder them.\n"
            "\n"
            "Arguments:\n"
            "1. height                          (numeric, optional, default=tip+1) the round n+1 to\n"
            "                                    audit. Must be <= tip+1: the beacon seed of a\n"
            ...
            "Result:\n"
            "{\n"
            "  \"delay\" : x,                       (numeric) D_i in seconds, the timer this score buys\n"
            "  \"score\" : x,                       (numeric) the raw score, null when ineligible\n"
            "  \"score_norm\" : x,                  (numeric) 1 - exp(-W * score_i), in (0,1)\n"
            "  \"effective_weight\" : n,            (numeric) w_eff_i = w_i * Psi_i\n"
            "  \"eligible\" : true|false            (boolean) whether w_eff_i > 0,\n"
            "  \"address\" : \"address\",             (string) this node's validator address\n"
            "  \"target_block_time\" : n,           (numeric) T_block, the chain target-block-time\n"
            "  \"delta\" : x,                       (numeric) band half-width as a fraction of T_block\n"
            "  \"lambda\" : x,                      (numeric) gain of the global feedback\n"
            "  \"feedback\" : x,                    (numeric) Phi^(n), the correction common to all\n"
            "  \"height\" : n,                      (numeric) the round n+1 these values are for\n"
            "  \"seed\" : \"hex\",                    (string) the 32-byte seed the scores are drawn over\n"
            "  \"seed_source\" : \"randao|prevblockhash\", (string) which beacon produced it\n"
            "  \"dumping_function\" : \"none|sqrt|log\",   (string) g, the active dumping function\n"
            "  \"total_effective_weight\" : x       (numeric) W = sum_j g(w_eff_j), the band normalizer\n"
            "}\n"
            + HelpExampleCli("wpoagetlocaldelay", "")
            + HelpExampleRpc("wpoagetlocaldelay", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetnodedelay",
            "wpoagetnodedelay \"address\" ( height )\n"
            "\n"
            "Returns one validator's mining delay for the round at `height`.\n"
            "\n"
            "D_i = T_block + delta*T_block*(2*score_norm_i - 1) + lambda*Phi^(n). The middle\n"
            "term orders the candidates and only that; Phi is identical for every candidate of\n"
            "the round, so it shifts all timers together and can never reorder them.\n"
            ...
            "  \"total_effective_weight\" : x       (numeric) W = sum_j g(w_eff_j), the band normalizer\n"
            "}\n"
            + HelpExampleCli("wpoagetnodedelay", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("wpoagetnodedelay", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("wpoalistdelays",
            "wpoalistdelays ( height )\n"
            "\n"
            "Returns the mining delay of EVERY validator for the round at `height`, in\n"
            ...
            "{\n"
            "  \"validators\" : n,                  (numeric) how many validators were scored\n"
            "  \"delays\" : {                       (object) address -> delay entry\n"
            "    \"address\" : {\n"
            "      \"delay\" : x, \"score\" : x, \"score_norm\" : x,\n"
            "      \"effective_weight\" : n, \"eligible\" : true|false\n"
            "    }, ...\n"
            "  },\n"
            "  \"target_block_time\" : n,           (numeric) T_block, the chain target-block-time\n"
            "  \"delta\" : x,                       (numeric) band half-width as a fraction of T_block\n"
            ...
            "  \"total_effective_weight\" : x       (numeric) W = sum_j g(w_eff_j), the band normalizer\n"
            "}\n"
            + HelpExampleCli("wpoalistdelays", "")
            + HelpExampleRpc("wpoalistdelays", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetblocksortition",
            "wpoagetblocksortition height\n"
            "\n"
            "Returns the REAL, private sortition score of the validator that WON the round at\n"
            "`height`, recomputed from the VRF reveal its block carries.\n"
            "\n"
            "This is the only wpoa* command that reports the score the election actually ran\n"
            "on. The wpoa*score / wpoa*delay families score candidates with the PUBLIC\n"
            "Efraimidis form HMAC-SHA256(seed, address), which is all that is computable for a\n"
            "validator whose secret key this node does not hold; under private sortition the\n"
            "election draws from a VRF under the proposer's own key, so the public form is a\n"
            "model audit and is statistically unrelated to the real draw. The winner is the one\n"
            "candidate whose real score IS publicly recomputable, because its block publishes\n"
            "the reveal -- and that is the same recomputation VerifyBlockMinerWPoA performs to\n"
            "enforce the time bar when the block is accepted.\n"
            "\n"
            "Weights are read as of the current tip: the registry has no height-bound read, and\n"
            "they only change on an epoch boundary, so the answer is exact whenever\n"
            "sample_epoch equals epoch, and `weight_epoch_stale` marks the rows where it is not.\n"
            "\n"
            "Arguments:\n"
            "1. height                          (numeric, required) a height on the active chain.\n"
            "\n"
            "Result:\n"
            "{\n"
            "  \"height\" : n,                      (numeric) the audited height\n"
            "  \"hash\" : \"hash\",                   (string) the block at that height\n"
            "  \"miner\" : \"address\",               (string) the validator that signed it\n"
            "  \"score\" : x,                       (numeric) its REAL score -ln(u)/g(w_eff), u from the VRF reveal\n"
            "  \"score_norm\" : x,                  (numeric) 1 - exp(-W*score); U(0,1) for a true argmin\n"
            "  \"delay\" : x,                       (numeric) D_i in seconds the score entitled it to\n"
            "  \"earliest_time\" : n,               (numeric) parent_time + floor(delay), the bar enforced\n"
            "  \"block_time\" : n,                  (numeric) the block's own header timestamp\n"
            "  \"parent_time\" : n,                 (numeric) the parent's header timestamp\n"
            "  \"dt_prev\" : n,                     (numeric) block_time - parent_time\n"
            "  \"time_received\" : x,               (numeric) LOCAL sub-second arrival time on this node\n"
            "  \"effective_weight\" : n,            (numeric) w_eff of the signer for this round\n"
            "  \"total_effective_weight\" : x,      (numeric) W = sum_j g(w_eff_j)\n"
            "  \"target_block_time\" : n, \"delta\" : x, \"lambda\" : x, \"feedback\" : x,\n"
            "  \"seed\" : \"hex\", \"seed_source\" : \"randao|prevblockhash\",\n"
            "  \"dumping_function\" : \"none|sqrt|log\",\n"
            "  \"epoch\" : n,                       (numeric) epoch of the audited height\n"
            "  \"sample_height\" : n,               (numeric) the tip this was computed against\n"
            "  \"sample_epoch\" : n,                (numeric) epoch of that tip\n"
            "  \"weight_epoch_stale\" : true|false, (boolean) sample_epoch != epoch: weights may differ\n"
            "  \"verdict\" : \"ok|not-sortition|no-reveal|no-signer|no-weight|no-block|no-block-data|unevaluable\"\n"
            "}\n"
            + HelpExampleCli("wpoagetblocksortition", "1000")
            + HelpExampleRpc("wpoagetblocksortition", "1000")
        ));

    mapHelpStrings.insert(std::make_pair("wpoalistblocksortition",
            "wpoalistblocksortition block-set-identifier\n"
            "\n"
            "As wpoagetblocksortition, for every block of a set: the REAL private score of each\n"
            "round's winner, recomputed from the VRF reveal its block carries.\n"
            "\n"
            "The block set uses the same grammar as listblocks, so an observer can pass the\n"
            "window it already built for that call. Every block is read from disk, so one call\n"
            "is capped at 1000 heights.\n"
            "\n"
            "Arguments:\n"
            "1. block-set-identifier            (string, required) one of:\n"
            "                                    \"block-height\"  -1 means the last block\n"
            "                                    \"block-hash\"\n"
            "                                    \"block-height-block-height\"  a range\n"
            "                                    \"block-height-block-count\"   e.g. \"100-199\"\n"
            "\n"
            "Result:\n"
            "[                                   (array) one wpoagetblocksortition object per block\n"
            "  { \"height\" : n, \"miner\" : \"address\", \"score\" : x, \"delay\" : x, ... },\n"
            "  ...\n"
            "]\n"
            + HelpExampleCli("wpoalistblocksortition", "\"1000-1099\"")
            + HelpExampleRpc("wpoalistblocksortition", "\"1000-1099\"")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetlocaleffectiveweight",
            "wpoagetlocaleffectiveweight ( height )\n"
            "\n"
            "Returns g(w) for this node: its raw wPoA weight after the dumping function.\n"
            "\n"
            "The DUMPING STAGE IN ISOLATION: g applied to the raw registry weight, before any\n"
            "behavioural correction. For the weight the election actually consumes — malus\n"
            "first, then dumping — use wpoagetnodefinalweight.\n"
            "\n"
            "Arguments:\n"
            "1. height                          (numeric, optional, default=tip+1) the round n+1 to\n"
            "                                    audit. Must be <= tip+1: the beacon seed of a\n"
            ...
            "Result:\n"
            "{\n"
            "  \"raw_weight\" : n,                  (numeric) w_i, straight off wpoa-weights\n"
            "  \"dumping_function\" : \"none|sqrt|log\", (string) g, the active dumping function\n"
            "  \"effective_weight\" : x             (numeric) g(w_i),\n"
            "  \"address\" : \"address\",             (string) this node's validator address\n"
            "  \"height\" : n                       (numeric) the round the weight was read for\n"
            "}\n"
            + HelpExampleCli("wpoagetlocaleffectiveweight", "")
            + HelpExampleRpc("wpoagetlocaleffectiveweight", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetnodeeffectiveweight",
            "wpoagetnodeeffectiveweight \"address\" ( height )\n"
            "\n"
            "Returns g(w) for one validator: its raw wPoA weight after the dumping function.\n"
            "\n"
            "The DUMPING STAGE IN ISOLATION: g applied to the raw registry weight, before any\n"
            "behavioural correction. For the weight the election actually consumes — malus\n"
            "first, then dumping — use wpoagetnodefinalweight.\n"
            ...
            "  \"height\" : n                       (numeric) the round the weight was read for\n"
            "}\n"
            + HelpExampleCli("wpoagetnodeeffectiveweight", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("wpoagetnodeeffectiveweight", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("wpoalisteffectiveweights",
            "wpoalisteffectiveweights ( height )\n"
            "\n"
            "Returns g(w) for EVERY validator in the weight registry, in ascending address order.\n"
            "\n"
            "The DUMPING STAGE IN ISOLATION: g applied to the raw registry weight, before any\n"
            "behavioural correction. For the weight the election actually consumes — malus\n"
            "first, then dumping — use wpoagetnodefinalweight.\n"
            ...
            "Result:\n"
            "{\n"
            "  \"height\" : n,                      (numeric) the round the weights were read for\n"
            "  \"dumping_function\" : \"none|sqrt|log\", (string) g, the active dumping function\n"
            "  \"validators\" : n,                  (numeric) how many validators are registered\n"
            "  \"effective_weights\" : {            (object) address -> { raw_weight, effective_weight }\n"
            "    \"address\" : { \"raw_weight\" : n, \"dumping_function\" : \"...\", \"effective_weight\" : x },\n"
            "    ...\n"
            "  }\n"
            "}\n"
            + HelpExampleCli("wpoalisteffectiveweights", "")
            + HelpExampleRpc("wpoalisteffectiveweights", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetlocalfinalweight",
            "wpoagetlocalfinalweight ( height )\n"
            "\n"
            "Returns the weight the election actually consumes for this node at `height`:\n"
            "g(w * Psi) — the raw registry weight corrected by the behavioural malus, then damped.\n"
            "\n"
            "ORDER MATTERS: the behavioural correction is applied to the RAW weight first, and\n"
            "the dumping function to the product — g(w_i * Psi_i). A result of 0 is a legitimate\n"
            "state, not an error: the validator is simply outside V+ for this round, with\n"
            "election probability 0, and re-enters automatically once M decays below M_max.\n"
            "\n"
            "Arguments:\n"
            "1. height                          (numeric, optional, default=tip+1) the round n+1 to\n"
            "                                    audit. Must be <= tip+1: the beacon seed of a\n"
            ...
            "{\n"
            "  \"raw_weight\" : n,                  (numeric) w_i, straight off wpoa-weights\n"
            "  \"malus\" : x,                       (numeric) M_i, the accumulated severity\n"
            "  \"malus_factor\" : x,                (numeric) Psi_i = max(0, 1 - M_i/M_max), in [0,1]\n"
            "  \"weight_after_malus\" : n,          (numeric) w_i * Psi_i\n"
            "  \"dumping_function\" : \"none|sqrt|log\", (string) g, the active dumping function\n"
            "  \"effective_weight_after_malus_and_dumping\" : x, (numeric) g(w_i * Psi_i)\n"
            "  \"eligible\" : true|false            (boolean) false when the product is 0,\n"
            "  \"address\" : \"address\",             (string) this node's validator address\n"
            "  \"height\" : n,                      (numeric) the round the weight was read for\n"
            "  \"malus_epoch\" : n                  (numeric) epoch(height)-1, whose M governs this round\n"
            "}\n"
            + HelpExampleCli("wpoagetlocalfinalweight", "")
            + HelpExampleRpc("wpoagetlocalfinalweight", "")
        ));

    mapHelpStrings.insert(std::make_pair("wpoagetnodefinalweight",
            "wpoagetnodefinalweight \"address\" ( height )\n"
            "\n"
            "Returns the weight the election actually consumes for one validator at `height`:\n"
            "g(w * Psi).\n"
            "\n"
            "ORDER MATTERS: the behavioural correction is applied to the RAW weight first, and\n"
            "the dumping function to the product — g(w_i * Psi_i). A result of 0 is a legitimate\n"
            ...
            "  \"malus_epoch\" : n                  (numeric) epoch(height)-1, whose M governs this round\n"
            "}\n"
            + HelpExampleCli("wpoagetnodefinalweight", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("wpoagetnodefinalweight", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("wpoalistfinalweights",
            "wpoalistfinalweights ( height )\n"
            "\n"
            "Returns g(w * Psi) for EVERY validator, in ascending address order. The malus\n"
            "accumulators are folded once for the whole map rather than per address.\n"
            "\n"
            "ORDER MATTERS: the behavioural correction is applied to the RAW weight first, and\n"
            "the dumping function to the product — g(w_i * Psi_i). A result of 0 is a legitimate\n"
            "state, not an error: the validator is simply outside V+ for this round, with\n"
            ...
            "{\n"
            "  \"height\" : n,                      (numeric) the round the weights were read for\n"
            "  \"malus_epoch\" : n,                 (numeric) epoch(height)-1, whose M governs the round\n"
            "  \"dumping_function\" : \"none|sqrt|log\", (string) g, the active dumping function\n"
            "  \"validators\" : n,                  (numeric) how many validators are registered\n"
            "  \"final_weights\" : {                (object) address -> final-weight entry\n"
            "    \"address\" : {\n"
            "      \"raw_weight\" : n, \"malus\" : x, \"malus_factor\" : x, \"weight_after_malus\" : n,\n"
            "      \"effective_weight_after_malus_and_dumping\" : x, \"eligible\" : true|false\n"
            "    }, ...\n"
            "  }\n"
            "}\n"
            + HelpExampleCli("wpoalistfinalweights", "")
            + HelpExampleRpc("wpoalistfinalweights", "")
        ));

    // --- WeightEngine epoch audit: one family per thesis definition ---
    mapHelpStrings.insert(std::make_pair("weightgetlocalcontribution",
            "weightgetlocalcontribution ( epoch )\n"
            "\n"
            "Returns this node's weighted contribution c_i to its cluster, for `epoch`.\n"
            "\n"
            "c_i = ESG_i * tau_i / kappa (Def. contributo-pesato). A MINER's own activity does\n"
            "NOT pass through c_i: it enters its cluster's raw weight directly as tau_Mk, which\n"
            "weightgetnodeclusterweight shows. An address that declared no cluster is reported\n"
            "with member=false and a contribution of 0 rather than as an error.\n"
            "\n"
            "Every value is read off the SAME per-epoch fold the publishing thread and the\n"
            "independent verifier run, so an audit can never disagree with the weight this\n"
            "node published.\n"
            "\n"
            "Arguments:\n"
            "1. epoch                           (numeric, optional, default=newest buried epoch)\n"
            "                                    the epoch to audit. An epoch that is not yet\n"
            "                                    buried is refused with \"epoch not yet\n"
            "                                    finalized\": a shallow reorg could still change it.\n"
            "\n"
            "Result:\n"
            "{\n"
            "  \"address\" : \"address\",             (string) the company the contribution is for\n"
            "  \"cluster\" : \"address\",             (string) the miner whose cluster it declared\n"
            "  \"esg_score\" : x,                   (numeric) ESG_i, certified on weight-engine-esg\n"
            "  \"activity\" : n,                    (numeric) tau_i, confirmed txs it signed this epoch\n"
            "  \"kappa\" : x,                       (numeric) the network normalization constant\n"
            "  \"contribution\" : x,                (numeric) c_i = ESG_i * tau_i / kappa\n"
            "  \"member\" : true|false,             (boolean) whether it declared any cluster\n"
            "  \"epoch\" : n                        (numeric) the epoch audited\n"
            "}\n"
            + HelpExampleCli("weightgetlocalcontribution", "")
            + HelpExampleRpc("weightgetlocalcontribution", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetnodecontribution",
            "weightgetnodecontribution \"address\" ( epoch )\n"
            "\n"
            "Returns one company's weighted contribution c_i to its cluster, for `epoch`.\n"
            "\n"
            "c_i = ESG_i * tau_i / kappa (Def. contributo-pesato). A MINER's own activity does\n"
            "NOT pass through c_i: it enters its cluster's raw weight directly as tau_Mk, which\n"
            "weightgetnodeclusterweight shows. An address that declared no cluster is reported\n"
            ...
            "\n"
            "Arguments:\n"
            "1. \"address\"                        (string, required) the company / node address\n"
            "2. epoch                           (numeric, optional, default=newest buried epoch)\n"
            "                                    the epoch to audit. An epoch that is not yet\n"
            "                                    buried is refused with \"epoch not yet\n"
            "                                    finalized\": a shallow reorg could still change it.\n"
            "\n"
            ...
            "  \"epoch\" : n                        (numeric) the epoch audited\n"
            "}\n"
            + HelpExampleCli("weightgetnodecontribution", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("weightgetnodecontribution", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("weightlistcontributions",
            "weightlistcontributions ( epoch )\n"
            "\n"
            "Returns c_i for EVERY declared cluster member in `epoch`, grouped by address.\n"
            "\n"
            "c_i = ESG_i * tau_i / kappa (Def. contributo-pesato). A MINER's own activity does\n"
            "NOT pass through c_i: it enters its cluster's raw weight directly as tau_Mk, which\n"
            "weightgetnodeclusterweight shows. An address that declared no cluster is reported\n"
            ...
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"kappa\" : x,                       (numeric) the network normalization constant\n"
            "  \"companies\" : n,                   (numeric) how many members were found\n"
            "  \"contributions\" : {                (object) address -> contribution entry\n"
            "    \"address\" : {\n"
            "      \"cluster\" : \"address\", \"esg_score\" : x, \"activity\" : n, \"contribution\" : x\n"
            "    }, ...\n"
            "  }\n"
            "}\n"
            + HelpExampleCli("weightlistcontributions", "")
            + HelpExampleRpc("weightlistcontributions", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetlocalclusterweight",
            "weightgetlocalclusterweight ( epoch )\n"
            "\n"
            "Returns the raw and final weight of the cluster headed by THIS node, for `epoch`.\n"
            "\n"
            "Both stages are returned so the feedback can be audited on its own: at epoch 1\n"
            "w_k = W_k by construction and previous_epoch_return_rate is null; from epoch 2 the\n"
            "bracket lies in [1-lambda_w, 1], which is why lambda_w < 1 keeps w_k > 0 whenever\n"
            "W_k > 0.\n"
            "\n"
            "Every value is read off the SAME per-epoch fold the publishing thread and the\n"
            "independent verifier run, so an audit can never disagree with the weight this\n"
            ...
            "Result:\n"
            "{\n"
            "  \"miner\" : \"address\",               (string) the cluster head\n"
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"miner_esg_score\" : x,             (numeric) ESG_Mk\n"
            "  \"miner_activity\" : n,              (numeric) tau_Mk, the miner's own activity\n"
            "  \"companies\" : n,                   (numeric) how many members the cluster has\n"
            "  \"companies_contribution_sum\" : x,  (numeric) sum_i c_i over the members\n"
            "  \"kappa\" : x,                       (numeric) the network normalization constant\n"
            "  \"raw_cluster_weight\" : x,          (numeric) W_k = ESG_Mk * (tau_Mk + sum_i c_i)\n"
            "  \"previous_epoch_return_rate\" : x,  (numeric) phi_k of epoch e-1; null when e = 1\n"
            "  \"lambda_w\" : x,                    (numeric) the inter-epoch feedback damping\n"
            "  \"final_cluster_weight\" : x,        (numeric) w_k = W_k*(phi_prev*lambda_w + 1-lambda_w)\n"
            "  \"published_weight\" : n             (numeric) the integer actually put on wpoa-weights\n"
            "}\n"
            + HelpExampleCli("weightgetlocalclusterweight", "")
            + HelpExampleRpc("weightgetlocalclusterweight", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetnodeclusterweight",
            "weightgetnodeclusterweight \"miner_address\" ( epoch )\n"
            "\n"
            "Returns the raw and final weight of one cluster, for `epoch`.\n"
            "\n"
            "Both stages are returned so the feedback can be audited on its own: at epoch 1\n"
            "w_k = W_k by construction and previous_epoch_return_rate is null; from epoch 2 the\n"
            "bracket lies in [1-lambda_w, 1], which is why lambda_w < 1 keeps w_k > 0 whenever\n"
            ...
            "\n"
            "Arguments:\n"
            "1. \"miner_address\"                  (string, required) the cluster head to look up\n"
            "2. epoch                           (numeric, optional, default=newest buried epoch)\n"
            "                                    the epoch to audit. An epoch that is not yet\n"
            "                                    buried is refused with \"epoch not yet\n"
            "                                    finalized\": a shallow reorg could still change it.\n"
            ...
            "  \"published_weight\" : n             (numeric) the integer actually put on wpoa-weights\n"
            "}\n"
            + HelpExampleCli("weightgetnodeclusterweight", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("weightgetnodeclusterweight", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("weightlistclusterweights",
            "weightlistclusterweights ( epoch )\n"
            "\n"
            "Returns the raw and final weight of EVERY cluster in `epoch`, by miner address.\n"
            "\n"
            "Both stages are returned so the feedback can be audited on its own: at epoch 1\n"
            "w_k = W_k by construction and previous_epoch_return_rate is null; from epoch 2 the\n"
            "bracket lies in [1-lambda_w, 1], which is why lambda_w < 1 keeps w_k > 0 whenever\n"
            ...
            "  \"kappa\" : x,                       (numeric) the network normalization constant\n"
            "  \"lambda_w\" : x,                    (numeric) the inter-epoch feedback damping\n"
            "  \"clusters\" : n,                    (numeric) how many clusters exist\n"
            "  \"cluster_weights\" : {              (object) miner -> cluster-weight entry\n"
            "    \"address\" : { \"raw_cluster_weight\" : x, \"final_cluster_weight\" : x, ... }, ...\n"
            "  }\n"
            "}\n"
            + HelpExampleCli("weightlistclusterweights", "")
            + HelpExampleRpc("weightlistclusterweights", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetlocalreturns",
            "weightgetlocalreturns ( epoch )\n"
            "\n"
            "Returns R_k for the cluster headed by THIS node in `epoch`: how much it paid to the\n"
            "network treasury.\n"
            "\n"
            "R_k is DERIVED from the blocks, never declared: it is the native-currency value paid\n"
            "to the treasury address by transactions the miner signed, among the epoch's confirmed\n"
            "transactions. With no -weighttreasuryaddress configured it is uniformly 0 on every\n"
            "node — a legitimate configuration, not an error.\n"
            "\n"
            "Every value is read off the SAME per-epoch fold the publishing thread and the\n"
            "independent verifier run, so an audit can never disagree with the weight this\n"
            "node published.\n"
            ...
            "  \"miner\" : \"address\",               (string) the cluster head\n"
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"treasury_address\" : \"address\",    (string) empty when none is configured\n"
            "  \"returns\" : x                      (numeric) R_k, in native currency\n"
            "}\n"
            + HelpExampleCli("weightgetlocalreturns", "")
            + HelpExampleRpc("weightgetlocalreturns", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetnodereturns",
            "weightgetnodereturns \"miner_address\" ( epoch )\n"
            "\n"
            "Returns R_k for one cluster in `epoch`: how much it paid to the network treasury.\n"
            "\n"
            "R_k is DERIVED from the blocks, never declared: it is the native-currency value paid\n"
            "to the treasury address by transactions the miner signed, among the epoch's confirmed\n"
            "transactions. With no -weighttreasuryaddress configured it is uniformly 0 on every\n"
            ...
            "  \"returns\" : x                      (numeric) R_k, in native currency\n"
            "}\n"
            + HelpExampleCli("weightgetnodereturns", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("weightgetnodereturns", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("weightlistreturns",
            "weightlistreturns ( epoch )\n"
            "\n"
            "Returns R_k for EVERY cluster in `epoch`, by miner address.\n"
            "\n"
            "R_k is DERIVED from the blocks, never declared: it is the native-currency value paid\n"
            "to the treasury address by transactions the miner signed, among the epoch's confirmed\n"
            "transactions. With no -weighttreasuryaddress configured it is uniformly 0 on every\n"
            ...
            "  \"treasury_address\" : \"address\",    (string) empty when none is configured\n"
            "  \"clusters\" : n,                    (numeric) how many clusters exist\n"
            "  \"returns\" : { \"address\" : x, ... } (object) miner -> R_k\n"
            "}\n"
            + HelpExampleCli("weightlistreturns", "")
            + HelpExampleRpc("weightlistreturns", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetlocalearnings",
            "weightgetlocalearnings ( epoch )\n"
            "\n"
            "Returns the gain g_k and the restitution rate phi_k of the cluster headed by THIS\n"
            "node, for `epoch`, with the flows they are derived from.\n"
            "\n"
            "Uscite EXCLUDES this epoch's own restitution, which is why expenses_excl_return is\n"
            "reported separately: were R_k also subtracted, saldo_k would shrink by the very\n"
            "restitution phi_k divides by, and a cluster that returned everything would score 0\n"
            "for the behaviour the mechanism exists to reward. A non-positive saldo yields\n"
            "phi_k = 0 — the defined value, never a division by zero.\n"
            "\n"
            "Every value is read off the SAME per-epoch fold the publishing thread and the\n"
            "independent verifier run, so an audit can never disagree with the weight this\n"
            "node published.\n"
            ...
            "  \"miner\" : \"address\",               (string) the cluster head\n"
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"income\" : x,                      (numeric) Entrate_k: every credit, coinbase included\n"
            "  \"expenses_gross\" : x,              (numeric) every debit, restitution still inside\n"
            "  \"expenses_excl_return\" : x,        (numeric) Uscite_k: the same, minus R_k\n"
            "  \"earnings\" : x,                    (numeric) g_k = Entrate_k - Uscite_k (may be negative)\n"
            "  \"return_amount\" : x,               (numeric) R_k, paid to the treasury this epoch\n"
            "  \"balance\" : x,                     (numeric) saldo_k = saldo_k(e-1) + g_k\n"
            "  \"return_rate\" : x                  (numeric) phi_k = R_k / saldo_k, in [0,1]\n"
            "}\n"
            + HelpExampleCli("weightgetlocalearnings", "")
            + HelpExampleRpc("weightgetlocalearnings", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetnodeearnings",
            "weightgetnodeearnings \"miner_address\" ( epoch )\n"
            "\n"
            "Returns the gain g_k and the restitution rate phi_k of one cluster, for `epoch`.\n"
            "\n"
            "Uscite EXCLUDES this epoch's own restitution, which is why expenses_excl_return is\n"
            "reported separately: were R_k also subtracted, saldo_k would shrink by the very\n"
            "restitution phi_k divides by, and a cluster that returned everything would score 0\n"
            ...
            "  \"return_rate\" : x                  (numeric) phi_k = R_k / saldo_k, in [0,1]\n"
            "}\n"
            + HelpExampleCli("weightgetnodeearnings", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("weightgetnodeearnings", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("weightlistearnings",
            "weightlistearnings ( epoch )\n"
            "\n"
            "Returns g_k, saldo_k and phi_k for EVERY cluster in `epoch`, by miner address.\n"
            "\n"
            "Uscite EXCLUDES this epoch's own restitution, which is why expenses_excl_return is\n"
            "reported separately: were R_k also subtracted, saldo_k would shrink by the very\n"
            "restitution phi_k divides by, and a cluster that returned everything would score 0\n"
            ...
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"clusters\" : n,                    (numeric) how many clusters exist\n"
            "  \"earnings\" : {                     (object) miner -> earnings entry\n"
            "    \"address\" : {\n"
            "      \"income\" : x, \"expenses_gross\" : x, \"expenses_excl_return\" : x,\n"
            "      \"earnings\" : x, \"return_amount\" : x, \"balance\" : x, \"return_rate\" : x\n"
            "    }, ...\n"
            "  }\n"
            "}\n"
            + HelpExampleCli("weightlistearnings", "")
            + HelpExampleRpc("weightlistearnings", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetlocalbalance",
            "weightgetlocalbalance ( epoch )\n"
            "\n"
            "Returns saldo_k for the cluster headed by THIS node at the end of `epoch`.\n"
            "\n"
            "saldo_k is the RUNNING TOTAL folded from epoch 1 (saldo_k(0) = 0), not a balance\n"
            "lookup. Because each epoch's Uscite excludes that epoch's restitution, it equals the\n"
            "ledger balance PLUS every past R_k; reading the UTXO balance instead would shrink the\n"
            "denominator of phi_k by the cluster's own good behaviour, and would be current state\n"
            "rather than a fact about the epoch.\n"
            "\n"
            "Every value is read off the SAME per-epoch fold the publishing thread and the\n"
            "independent verifier run, so an audit can never disagree with the weight this\n"
            "node published.\n"
            ...
            "  \"miner\" : \"address\",               (string) the cluster head\n"
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"earnings\" : x,                    (numeric) g_k of this epoch\n"
            "  \"balance\" : x                      (numeric) saldo_k = saldo_k(e-1) + g_k\n"
            "}\n"
            + HelpExampleCli("weightgetlocalbalance", "")
            + HelpExampleRpc("weightgetlocalbalance", "")
        ));

    mapHelpStrings.insert(std::make_pair("weightgetnodebalance",
            "weightgetnodebalance \"miner_address\" ( epoch )\n"
            "\n"
            "Returns saldo_k for one cluster at the end of `epoch`.\n"
            "\n"
            "saldo_k is the RUNNING TOTAL folded from epoch 1 (saldo_k(0) = 0), not a balance\n"
            "lookup. Because each epoch's Uscite excludes that epoch's restitution, it equals the\n"
            "ledger balance PLUS every past R_k; reading the UTXO balance instead would shrink the\n"
            ...
            "  \"balance\" : x                      (numeric) saldo_k = saldo_k(e-1) + g_k\n"
            "}\n"
            + HelpExampleCli("weightgetnodebalance", "\"1AbcdEfgh...\"")
            + HelpExampleRpc("weightgetnodebalance", "\"1AbcdEfgh...\"")
        ));

    mapHelpStrings.insert(std::make_pair("weightlistbalances",
            "weightlistbalances ( epoch )\n"
            "\n"
            "Returns saldo_k for EVERY cluster at the end of `epoch`, by miner address.\n"
            "\n"
            "saldo_k is the RUNNING TOTAL folded from epoch 1 (saldo_k(0) = 0), not a balance\n"
            "lookup. Because each epoch's Uscite excludes that epoch's restitution, it equals the\n"
            "ledger balance PLUS every past R_k; reading the UTXO balance instead would shrink the\n"
            ...
            "  \"epoch\" : n,                       (numeric) the epoch audited\n"
            "  \"clusters\" : n,                    (numeric) how many clusters exist\n"
            "  \"balances\" : { \"address\" : x, ... } (object) miner -> saldo_k\n"
            "}\n"
            + HelpExampleCli("weightlistbalances", "")
            + HelpExampleRpc("weightlistbalances", "")
        ));
}
/* MCHN END */
...
void mc_InitRPCHelpMap()
{
    ...
    mc_InitRPCHelpMap24();
    mc_InitRPCHelpMap25();
    mc_InitRPCHelpMap26();   /* MCHN -- wPoA/weightengine audit RPCs */
